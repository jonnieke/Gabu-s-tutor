...
const IdleScreen: React.FC<IdleScreenProps> = ({ onStartScan, onUploadImage, onUploadAudio, onRecordedAudio, onOpenIllustrate, onOpenSettings, onQuickAsk, onOpenBookmarks, recentTopics = [], onSelectRecent }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center p-4 sm:p-6 md:p-8 h-full overflow-y-auto text-center pb-20 sm:pb-24">
        <div className="absolute top-3 right-3 sm:top-4 sm:right-4 flex items-center gap-2">
          ...
          <button onClick={onOpenSettings} className="p-2 rounded-full bg-white/80 hover:bg-white transition-colors touch-manipulation" aria-label="Settings">
            ...
            <GabuIcon className="w-20 h-20 sm:w-24 sm:h-24 text-purple-500 mb-4 mx-auto animate-gentle-bounce" />
            <h1 id="hero-heading" className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold mb-2 text-gray-800">Making homework easy, step-by-step.</h1>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 sm:gap-6 w-full mb-8">
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-4 sm:p-5 mb-8">
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-3 rounded-full border-2 border-gray-200 focus:outline-none focus:ring-2 focus:ring-purple-300"
                  aria-label="Ask a question"
                />
                <button onClick={() => submitQuickAsk()} aria-label="Send question" className="p-3 bg-purple-600 text-white rounded-full hover:bg-purple-700 transform active:scale-95">
                  <SendIcon className="w-6 h-6"/>
                </button>
              </div>
              <div className="flex flex-wrap gap-2 mt-3 justify-center">
                {suggestions.map((s, i) => (
                  <button key={i} onClick={() => submitQuickAsk(s)} className="px-3 py-1.5 text-sm rounded-full bg-gray-100 hover:bg-purple-100 text-gray-700 border border-gray-200">
                    {s}
                  </button>
                ))}
              </div>
            ...
            {progress && (
              <div className="mb-8 bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-6 max-w-md mx-auto">
                <h3 className="text-lg font-bold text-gray-800 mb-4">Your Learning Progress</h3>
            ...
            {recentTopics.length > 0 && (
              <div className="mt-8 sm:mt-10">
                <h2 className="text-lg sm:text-xl font-extrabold text-gray-800 mb-3 text-left">Recent topics</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3">
                  {recentTopics.map((t, idx) => (
                    <button key={idx} onClick={() => onSelectRecent?.(t)} className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-xl sm:rounded-2xl bg-white/80 backdrop-blur-sm shadow hover:shadow-md text-left touch-manipulation">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-purple-100 grid place-items-center flex-shrink-0">
                        <span className="text-xs sm:text-sm font-extrabold text-purple-600">{Math.min(99, (idx+1)*7)}%</span>
                      </div>
                      <span className="text-xs sm:text-sm text-gray-700 font-semibold line-clamp-1">{t}</span>
                    </button>
                  ))}
                </div>
              </div>
